...
| **Screens** | `src/client/src/screens/` | Routed top-level surfaces composed of organisms. | `Home`, `Library` (知识库 gallery), `Manage` (`/manage` console: 概览 health · 对话评估 eval+scorers+traces · 校准 cortex-tuning), `AuthGate` (remote-access token gate wrapping the app) |
