...
| **`fill_itinerary` as a script tool** — ❌ declined | S1 named it (a visa-shaped wrapper over the generic `pdf_fill`) as a candidate to stop being compiled C# and become "the first real proof of the capability model rather than an exception to it". | **The motivation was satisfied by other means.** S2a shipped that proof — `e2e-p38` runs a genuine hot-loaded Script capability with a real grant and a sandbox-escape battery — so this would buy a second demonstration at a real cost. A `Script` capability is off until a human enables it, so a working tool would go dark on every existing install while the shipped knowledge base still lists it; and because a grant's vocabulary is site-relative while the `pdf-form` leaf lives in the install's `res/`, the script would have to vendor pdf-lib + fontkit into every household's data folder and stop being updated with the app. 39 lines of C# is the cheaper, more maintainable shape. |
