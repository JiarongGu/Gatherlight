...
1. **Invoke the 5 core skills in parallel**:
   ...
   - `/tool-loader "task description"` — INVOKE/SKIP list of MCP tools (`scrape`, `extract`, `recall_facts`, …) exposed by the Gatherlight server
...
| `recall_facts` / `remember_fact` / `expand_fact` | `mcp__planner-tools__recall_facts` / `remember_fact` / `expand_fact` | **Cross-session fact memory** — the granular things you verified in an earlier session: a URL that resolved, a price with its date, a venue's status, a booking policy. **`recall_facts` BEFORE researching any of those** — a hit means you already did this work. Hits are ranked so what keeps proving useful comes first, and each carries a `ref` for `expand_fact`, which returns the full text plus the facts it is LINKED to (facts recalled together become linked, so this often surfaces something the query never matched). `remember_fact` stores one after you verify it. Policies and preferences do NOT belong here — those go in `household/` or `.claude/`; this is for what is too fine-grained to curate. |
...
| **verification scrapers** | `mcp__planner-tools__flight_prices` / `hotel_prices` / `restaurant_info` / `hotel_info` / `flight_schedule` / `policy_check` / `wiki_info` | Purpose-built lookups, each rendering the right sites in a real browser: fares for a route across dates · room rates by name + dates · whether a restaurant exists and which URL is really it · a hotel's address and phone cross-checked · a flight number and carrier code · visa and entry rules from official sources · an encyclopedia summary for a reference entity. **Prefer these over raw `scrape`** — they know which sources to trust and what a broken result looks like. |
| `budget_scan` | `mcp__planner-tools__budget_scan` | Honest totals straight from a budget file — arithmetic without a model, so the figures cannot drift. |
| `index_*` | `mcp__planner-tools__index_search` / `index_list` / `index_reindex` | Search and list the plan index (every plan file with its dates and type). `index_search` finds a past plan faster than a Grep sweep. |
...
- **The tool list above is what exists — the server provides it, and it is the ONLY way to run deterministic work.** There is no `tools/` directory to shell into here and no script to run: if a workspace has one, it is a leftover from an older layout, not a working tool. If `mcp__planner-tools__*` shows a tool, call it; if it doesn't, the capability is missing — say so and record it via `/remember`, rather than reaching for a filesystem path. (Ask the user before proposing a new tool: authoring one is a privileged action they have to approve.)
