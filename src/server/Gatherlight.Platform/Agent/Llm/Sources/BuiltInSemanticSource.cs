...
/// 222 MB of model against Ollama's 622 MB model plus its own runtime download. Measured 2026-08-22 it also
/// ties the Ollama arm on retrieval (8/8 top-1 on one fixture) and embeds ~21× faster in-process, with the
/// variant, tokenizer and prompting all chosen by measurement rather than by preference — see
/// <c>docs/builtin-model-runner.md</c> and <see cref="OnnxEmbedder"/>.</para>
///
...
public sealed class BuiltInSemanticSource : IMemorySemanticSource
{
    ...
    /// tag: nothing pulls this by name, so a model id here would imply a freedom that does not exist.</summary>
    private const string ModelId = "embeddinggemma-300m-onnx";
    ...
    public string Description =>
        ...
        + "实测检索质量与本机 Ollama 同分,速度更快;模型约 222 MB,在「资源 · Resources」面板下载一次。";
        ...
        Task.FromResult<IReadOnlyList<ModelOption>>(IsConfigured(ctx.Settings)
            ? new[]
            {
                new ModelOption(ModelId, "EmbeddingGemma 300M(内置)", Installed: true,
                    ...
                    Note: "与「本机 · Ollama」里推荐的是同一个模型,实测同分;在应用内直接运行。"),
            }
