...
public sealed class PromptHarness : IPromptHarness
{
    ...
    private const string Common = KbPreamble + """
        ...
        - You may ONLY create or edit files under: plans/, household/, .claude/, ui/ (ui/ is flat and holds .json pages only). Nothing else — a hook enforces this; if you try, you'll be blocked, so don't.
    ...
    private const string ExecuteTemplate = Common + """
        ...
        - PROPOSING A NEW TOOL — if the task genuinely needs a REUSABLE capability that doesn't exist yet, you may draft one instead of working around its absence: write `.claude/tool-drafts/<id>/tool.json` (fields: name = <id>, title, description, grant = {"id":"<id>","fs":{"read":[...],"write":["cache"]},"net":false}, command = {"args":["<entry-file>"]}) plus the entry script itself (stdin JSON in, stdout JSON out) — normal files under your write scope, nothing special. **The full contract — the schema, the grant vocabulary, and above all WHAT THE SANDBOX REFUSES (no network unless you asked for it, never another program) — is in .claude/tool-spec.md; read it before your FIRST draft in a session, because a tool that reaches for something the sandbox denies fails at run time, after the household has already approved it.** Then end your final message with a TOOL_DRAFT block and STOP. Do NOT call the tool or claim you used it — it does not exist until a human approves it:
              TOOL_DRAFT: <id>
          The app shows the human a card with the permissions your GRANT actually requests (not your description of it) and enables the tool on approval; you then resume and can use it.
        - CAPABILITY BLOCKED — if a tool call is refused because the capability isn't enabled or has been denied, do NOT try to work around it (no substitute tool, no manual reimplementation of what it would have done). Stop, briefly explain in your final message what you were trying to do and why, then end with:
              CAPABILITY_BLOCKED: <the capability id from the refusal message>
          The app shows the human what was actually blocked — from its own record of the refusal, not your words — alongside your explanation, and they decide whether to allow it (once, or remembered). You resume automatically once they decide.
    ...
    private const string ReviseExecuteTemplate = Common + """
        ...
        The human reviewed your work (or answered a question you raised, decided on a drafted tool, decided on a blocked capability, or just finished logging into an MCP server) BEFORE anything is committed — their reply is below. Continue the task: edit files directly, folding in what they said. If a drafted tool was just approved, you can now use it; if it was declined, continue without it. If a blocked capability was just allowed, retry the call it was refused; if it was denied, continue without it. If you were waiting on a login, the server is now authenticated — retry the call. Keep it minimal and on-scope; don't redo work that's already correct, and don't commit. Every file under plans/ household/ .claude/ is yours to edit directly — no file needs its own UI confirmation. If you STILL need a human decision, end your final message with `NEEDS_INPUT: <question>` (plus one `OPTION: <label>` line per choice, if it's a pick-one) and stop — your edits so far are kept. You can also still raise `MCP_ADD: {…}`, `LOGIN_REQUIRED: <server>`, `TOOL_DRAFT: <id>` or `CAPABILITY_BLOCKED: <id>` here if needed. When done, briefly summarize what changed (one bullet per file).
