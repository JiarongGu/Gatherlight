...
public sealed class RecallFactsTool : IGatherlightTool
{
    ...
    public string Description =>
        ...
        + "检索词也可以直接写这条事实『是关于什么的』,不必照搬原文用词 —— 例如问「证件」也能命中一条只写了护照到期的事实。";
    ...
    public async Task<string> RunAsync(JsonElement args, CancellationToken ct)
    {
        ...
        if (hits.Count > 0)
        {
            ...
            if (rows.Count > 0)
            {
                ...
                foreach (var (row, graphRef) in rows.Take(limit))
                {
                    ...
                    var o = Row(row);
                    o["ref"] = graphRef;
                    // Measured for EVERY graph row, including one reached through a subject handle: the
                    // engine ranks those candidates itself since Lyntai 3.1. (They were once appended here
                    // with no measurement and flagged matched:"subject" so a "0.0" would not read as decayed.)
        ...
        // cannot displace a ranked hit, and when
    ...
    internal static JsonObject Row(KnowledgeRow r) => new()
    {
        ...
        // reader is the same "bought and never consulted" shape as an embedding no recall channel reads,
        // just cheaper. Emitting it is the smaller of the two honest fixes, and it earns its place: a row
        // matched by TEXT carries no retrievability, so this is the only signal it has for how much use a
        // fact actually gets.
        ["used"] = r.Hits,
