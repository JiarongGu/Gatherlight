...
| [dev-conventions.md](dev-conventions.md) | Any server/client/tooling code change | Modules + async Dapper + FluentMigrator numbering + FTS5-trigram search, type naming (role suffixes, never `Dto`), claude-CLI spawn hygiene, scorer/eval DI-collection pattern, security/access-gate + TLS model, launcher + two-phase update packaging, data-folder discipline, devtools loop |
